...
- 当前电机 ID 和关节顺序已经明确为 1 到 12：
  - 左前：`1=hip`，`2=thigh`，`3=calf`
  - 右前：`4=hip`，`5=thigh`，`6=calf`
  - 左后：`7=hip`，`8=thigh`，`9=calf`
  - 右后：`10=hip`，`11=thigh`，`12=calf`
...
- CAN 通道到腿的实际接线映射。建议默认按 `can0=左前`、`can1=右前`、`can2=左后`、`can3=右后` 设计配置，但最终必须以实机布线为准。
- 关节方向符号。
- 关节零点 offset。
- 软件限位。
...
- 推荐每路 CAN 带一条腿的 3 个电机，关节顺序为 `hip/thigh/calf`。
- 推荐电机映射：
  - `can0`: 左前 `1/2/3`
  - `can1`: 右前 `4/5/6`
  - `can2`: 左后 `7/8/9`
